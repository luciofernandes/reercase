...
import rmm.diagram.edit.policies.ForeignKeyItemSemanticEditPolicy;
...
public class ForeignKeyEditPart extends ShapeNodeEditPart {
	...
	protected boolean addFixedChild(EditPart childEditPart) {
		if (childEditPart instanceof ForeignKeyIDNameEditPart) {
			((ForeignKeyIDNameEditPart) childEditPart)
	...
	public class ForeignKeyFigure extends RectangleFigure {
		...
		private void createContents() {
			...
			ForeignKey foreignKey = ((ForeignKey) ((Node) ForeignKeyEditPart.this
					.getModel()).getElement());

			if (fFigureForeignKeyLabelFigure == null) {

				Integer id = 1;

				if (ForeignKeyEditPart.this.getParent() != null) {

					id = 0;
					List<?> children = ForeignKeyEditPart.this.getParent()
							.getChildren();

					if (children != null) {
						for (Object object : children) {
							if (object instanceof ForeignKeyEditPart) {
								id++;
							}
						}
					}
				...
				String fk_name = foreignKey.getName();
				...
				if (foreignKey.getID() == null) {
					foreignKey.eSetDeliver(false);
					foreignKey.setID(ACRONOMO + id);
					foreignKey.eSetDeliver(true);
				}

				fFigureForeignKeyLabelFigure = new WrappingLabel();
				fFigureForeignKeyLabelFigure.setText(foreignKey.getID() + ":"
						+ fk_name);
	...
	private static final String ACRONOMO = "FK";
