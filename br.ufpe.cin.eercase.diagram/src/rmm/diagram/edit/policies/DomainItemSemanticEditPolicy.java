 ...
 * @generated
 */
public class DomainItemSemanticEditPolicy extends RmmBaseItemSemanticEditPolicy {
	...
	private void addDestroyChildNodesCommand(ICompositeCommand cmd) {
		...
		for (Iterator<?> nit = view.getChildren().iterator(); nit.hasNext();) {
			...
			switch (RmmVisualIDRegistry.getVisualID(node)) {
			case DomainDomainConstraintCheckDomainCompartmentEditPart.VISUAL_ID:
				for (Iterator<?> cit = node.getChildren().iterator(); cit
						.hasNext();) {
					Node cnode = (Node) cit.next();
					switch (RmmVisualIDRegistry.getVisualID(cnode)) {
