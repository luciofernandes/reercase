...
public class SchemaCanonicalEditPolicy extends CanonicalEditPolicy {
	...
	protected Set getFeaturesToSynchronize() {
		if (myFeaturesToSynchronize == null) {
			...
			myFeaturesToSynchronize.add(RmmPackage.eINSTANCE.getSchema_Table());
			myFeaturesToSynchronize.add(RmmPackage.eINSTANCE
					.getSchema_Assertion());
			myFeaturesToSynchronize
					.add(RmmPackage.eINSTANCE.getSchema_Domain());
		}
	...
	protected List getSemanticChildrenList() {
		...
		List<RmmNodeDescriptor> childDescriptors = RmmDiagramUpdater
				.getSchema_1000SemanticChildren(viewObject);
		for (RmmNodeDescriptor d : childDescriptors) {
	...
	protected boolean isOrphaned(Collection<EObject> semanticChildren,
			final View view) {
		if (isShortcut(view)) {
			return RmmDiagramUpdater.isShortcutOrphaned(view);
		}
	...
	private boolean isMyDiagramElement(View view) {
		...
		return visualID == TableEditPart.VISUAL_ID
				|| visualID == AssertionEditPart.VISUAL_ID
	...
	protected void refreshSemantic() {
		...
		for (View v : getViewChildren()) {
			if (isShortcut(v)) {
				if (RmmDiagramUpdater.isShortcutOrphaned(v)) {
					orphaned.add(v);
				}
				continue;
			}
		...
		for (Iterator<RmmNodeDescriptor> descriptorsIterator = childDescriptors
				.iterator(); descriptorsIterator.hasNext();) {
			RmmNodeDescriptor next = descriptorsIterator.next();
			String hint = RmmVisualIDRegistry.getType(next.getVisualID());
		...
		for (RmmNodeDescriptor next : childDescriptors) {
			String hint = RmmVisualIDRegistry.getType(next.getVisualID());
	...
	private Collection<IAdaptable> refreshConnections() {
		...
		Collection<RmmLinkDescriptor> linkDescriptors = collectAllLinks(
				getDiagram(), domain2NotationMap);
		Collection existingLinks = new LinkedList(getDiagram().getEdges());
		for (Iterator linksIterator = existingLinks.iterator(); linksIterator
				.hasNext();) {
			Edge nextDiagramLink = (Edge) linksIterator.next();
			int diagramLinkVisualID = RmmVisualIDRegistry
			...
			for (Iterator<RmmLinkDescriptor> linkDescriptorsIterator = linkDescriptors
					.iterator(); linkDescriptorsIterator.hasNext();) {
				RmmLinkDescriptor nextLinkDescriptor = linkDescriptorsIterator
						.next();
	 ...
	 * @generated
	 */
	private Collection<RmmLinkDescriptor> collectAllLinks(View view,
		...
		case TableEditPart.VISUAL_ID: {
			if (!domain2NotationMap.containsKey(view.getElement())) {
				result.addAll(RmmDiagramUpdater
						.getTable_2004ContainedLinks(view));
			}
			domain2NotationMap.putView(view.getElement(), view);
			break;
		}
		case AssertionEditPart.VISUAL_ID: {
			if (!domain2NotationMap.containsKey(view.getElement())) {
				result.addAll(RmmDiagramUpdater
						.getAssertion_2007ContainedLinks(view));
			}
			domain2NotationMap.putView(view.getElement(), view);
			break;
		}
		case DomainEditPart.VISUAL_ID: {
			if (!domain2NotationMap.containsKey(view.getElement())) {
				result.addAll(RmmDiagramUpdater
						.getDomain_2008ContainedLinks(view));
			}
			domain2NotationMap.putView(view.getElement(), view);
			break;
		}
		case PrimaryKeyEditPart.VISUAL_ID: {
			if (!domain2NotationMap.containsKey(view.getElement())) {
				result.addAll(RmmDiagramUpdater
						.getPrimaryKey_3022ContainedLinks(view));
			}
			domain2NotationMap.putView(view.getElement(), view);
			break;
		}
		...
		case ForeignKeyEditPart.VISUAL_ID: {
			if (!domain2NotationMap.containsKey(view.getElement())) {
				result.addAll(RmmDiagramUpdater
						.getForeignKey_3024ContainedLinks(view));
			}
			domain2NotationMap.putView(view.getElement(), view);
			break;
		}
		case CheckEditPart.VISUAL_ID: {
			if (!domain2NotationMap.containsKey(view.getElement())) {
				result.addAll(RmmDiagramUpdater
						.getCheck_3025ContainedLinks(view));
			}
			domain2NotationMap.putView(view.getElement(), view);
			break;
		}
		case TriggerEditPart.VISUAL_ID: {
			if (!domain2NotationMap.containsKey(view.getElement())) {
				result.addAll(RmmDiagramUpdater
						.getTrigger_3026ContainedLinks(view));
			}
			domain2NotationMap.putView(view.getElement(), view);
			break;
		}
		...
		case Check2EditPart.VISUAL_ID: {
			if (!domain2NotationMap.containsKey(view.getElement())) {
				result.addAll(RmmDiagramUpdater
						.getCheck_3021ContainedLinks(view));
			}
			domain2NotationMap.putView(view.getElement(), view);
			break;
		}
	...
	private Collection<IAdaptable> createConnections(
			Collection<RmmLinkDescriptor> linkDescriptors,
			Domain2Notation domain2NotationMap) {
		LinkedList<IAdaptable> adapters = new LinkedList<IAdaptable>();
		for (RmmLinkDescriptor nextLinkDescriptor : linkDescriptors) {
			...
			CreateConnectionViewRequest.ConnectionViewDescriptor descriptor = new CreateConnectionViewRequest.ConnectionViewDescriptor(
					...
					RmmVisualIDRegistry.getType(nextLinkDescriptor
	...
	protected final EditPart getHintedEditPart(EObject domainModelElement,
		...
		View view = (View) domain2NotationMap.getHinted(domainModelElement,
				RmmVisualIDRegistry.getType(hintVisualId));
		if (view != null) {
			return (EditPart) getHost().getViewer().getEditPartRegistry()
					.get(view);
		}
